...
final class StatementOp extends ProgramOp {
  ...
  ProgramDelta _execute(Evaluation evaluation, {required List<Statement> remove, required List<Statement> insert}) {
    ...
    final rewritten = _edit(evaluation, index, index + remove.length, insert);

    final derived = <StatementOp>[];
    for (final entry in rewritten.entries) {
      final id = entry.key;
      final (before, after) = entry.value;
      derived.add(.new(anchor: .at(id), removed: [before], inserted: after, derived: true));
    }
  ...
  ProgramOp? coalesce(ProgramOp next) {
    ...
    if (derived != next.derived) return null;
