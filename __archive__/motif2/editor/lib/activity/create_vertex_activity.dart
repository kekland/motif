...
class CreateVertexActivity extends DragActivity {
  ...
  void onStart(PositionedGestureDetails details) {
    ...
    if (existingTransientEdge != null) {
      ...
      final endVertex = editor.scene.topology.embedVertexAtHitTest(hitTest);
      transientEdge.end = endVertex.localToScene(endVertex.position);
    } else {
  ...
  void onUpdate(DragUpdateDetails details) {
    final position = editor.globalToScene(details.globalPosition.vec2);
  ...
  void onEnd(DragEndDetails? details) {
    ...
    if (!isNewEdge) {
      ...
      final endHitTest = editor.scene.hitTest(endPosition);
