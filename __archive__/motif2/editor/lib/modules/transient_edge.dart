...
class TransientEdge with ChangeNotifier, ChangeNotifierDisposable {
  ...
  Vector2 get start => startVertex.localToScene(startVertex.position);
  ...
  SceneNode get startParent => startVertex.parent!;
  ...
  List<Edge> _performCommit(SceneHitTestResult? endHitTest) {
    ...
    final parent = startVertex.lca(endVertex);
    final transform = parent.getTransformTo(null)..invert();
    final transformedCubic = cubic.transform(transform);

    return editor.scene.topology.commitStroke(
      parent,
      .fromCubic(transformedCubic),
  ...
  void _performRemove() {
    if (startVertex.degree == 0) startVertex.detach();
  }
}
