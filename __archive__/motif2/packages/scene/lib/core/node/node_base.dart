...
mixin SceneNodeBase implements SceneNode {
  ...
  void _attachToScene(Scene scene) {
    ...
    // Attaching order: vertices -> edges -> faces, then objects
    for (final v in children.whereType<Vertex>()) v._attachToScene(scene);
    for (final e in children.whereType<Edge>()) e._attachToScene(scene);
    for (final f in children.whereType<Face>()) f._attachToScene(scene);
    for (final o in children.where((c) => c is! Cell)) o._attachToScene(scene);
  }
  ...
  void _detachFromScene() {
    ...
    // Detaching order: faces -> edges -> vertices, then objects
    for (final o in children.where((c) => c is! Cell)) o._detachFromScene();
    for (final f in children.whereType<Face>()) f._detachFromScene();
    for (final e in children.whereType<Edge>()) e._detachFromScene();
    for (final v in children.whereType<Vertex>()) v._detachFromScene();
  ...
  @override
  bool get isAttached => _scene != null;
  ...
  SceneNode lca(SceneNode other) {
    ...
    throw StateError('No common ancestor found between $this and $other');
  }
  ...
  @override
  set constraints(covariant LayoutConstraints? value) {
  ...
  void layout([covariant LayoutConstraints? constraints]) {
    ...
    if (!_needsLayout) return;
    if (_constraints == null) throw StateError('Cannot layout node without constraints');
    _needsLayout = false;
    performLayout(_constraints!);
  }
  ...
  SceneNodeNotifier? _notifier;

  @override
  ChangeNotifier call([NodeUpdateAspect aspect = .all]) {
    _notifier = _notifier ?? _scene!.notifierForNode(id);
    if (aspect == .all) return _notifier!;
    return _notifier!.aspect(aspect);
  }
