...
GN = SKIA_SRC / 'bin' / 'gn'
FETCH_NINJA = SKIA_SRC / 'bin' / 'fetch-ninja'
NINJA = SKIA_SRC / 'third_party' / 'ninja' / 'ninja'
...
def lib_dir(platform: str, config: str) -> Path: return LIBS_DIR / PLATFORMS[platform]['out_name'] / config
...
def fetch_ninja():
  subprocess.run([str(FETCH_NINJA)], check=True)
...
def gn_gen(platform: str, config: str):
  ...
  subprocess.run(
    [str(GN), 'gen', str(out), f'--args={args}'],
    cwd=SKIA_SRC,
    check=True,
  )
...
def ninja_build(platform: str, config: str):
  ...
  subprocess.run(
    [str(NINJA), '-C', str(out), *NINJA_TARGETS],
    check=True,
  )
...
def main():
  ...
  print(f'  static libs:     lib/{out_name}/{config}/lib*.a')
  print(f'  public headers:  include/')
  print(f'  private headers: include/private')
  print(f'  icu data:        lib/share/icudtl.dat')
