...
sealed class ContainerChildLayout {
  ...
  ResolvedSize layout(ObjectSize size, LayoutConstraints constraints, List<SceneObject> children);
}
...
enum FlexDirection {
  ...
  ({double min, double max}) mainConstraints(LayoutConstraints constraints) => switch (this) {
    .row => (min: constraints.minWidth, max: constraints.maxWidth),
    .column => (min: constraints.minHeight, max: constraints.maxHeight),
  };

  ({double min, double max}) crossConstraints(LayoutConstraints constraints) => switch (this) {
    .row => (min: constraints.minHeight, max: constraints.maxHeight),
    .column => (min: constraints.minWidth, max: constraints.maxWidth),
  };

  LayoutConstraints tightConstraintsFor({required double main, required double cross}) => switch (this) {
