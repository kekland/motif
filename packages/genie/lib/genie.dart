...
extension _OffsetExt on ui.Offset {
  Vec2 get v2 => Vec2(dx, dy);
}

(Cubic2, Cubic2, Vec2, Vec2, Vec2) _generateCubics(ui.Rect src, ui.Rect dst) {
  final centerDx = dst.center.dx - src.center.dx;
  final centerDy = dst.center.dy - src.center.dy;

  late final (Vec2, Vec2) srcEdge;
  late final (Vec2, Vec2) dstEdge;
  ...
  final m = Vec2(dx, dy);
  Cubic2 smooth(Vec2 s, Vec2 p) => .new(
    s,
    p,
    p1: s + m * (1 / 3),
    p2: p - m * (1 / 3),
