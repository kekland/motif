...
class _UseValueListenableHook extends _ListenableHook {
  const _UseValueListenableHook(ValueListenable<Object?> super.animation);
...
class _ListenableStateHook extends HookState<void, _ListenableHook> {
  ...
  @override
  void build(flutter.BuildContext context) {}
...
class _ValueNotifierHook<T> extends Hook<ValueNotifier<T>> {
  const _ValueNotifierHook({super.keys, required this.initialData});

  final T initialData;

  @override
  _UseValueNotifierHookState<T> createState() => _UseValueNotifierHookState<T>();
}

class _UseValueNotifierHookState<T> extends HookState<ValueNotifier<T>, _ValueNotifierHook<T>> {
  late final notifier = ValueNotifier<T>(hook.initialData);

  @override
  ValueNotifier<T> build(flutter.BuildContext context) {
    return notifier;
  }
...
class _OnListenableChangeHookState extends HookState<void, _OnListenableChangeHook> {
  @override
  void initHook() {
    super.initHook();
    hook.listenable?.addListener(_listener);
