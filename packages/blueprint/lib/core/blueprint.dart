...
class Blueprint with ChangeNotifier, ChangeNotifierDisposable {
  ...
  final Map<Node, Vec2> _nodePositions;
  final Map<Node, bool> _nodeStatic;

  void addNode(Node node, {Vec2? position, bool isStatic = false}) {
    nodes.add(node);
  ...
  Vec2 getNodePosition(Node node) => _nodePositions[node]!;
  void setNodePosition(Node node, Vec2 position) {
