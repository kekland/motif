...
''';
  ...
  final errorListener = dawn.UncapturedErrorCallbackListener((device, type, str) {
    print('Error (${device}): $type, $str');
  });

  final instance = dawn.Instance.create();
  final adapter = await instance.requestAdapter();
  final device = adapter.createDevice(
    .new(
      label: 'main',
      uncapturedErrorCallbackInfo: .new(callback: errorListener),
    ),
  );
