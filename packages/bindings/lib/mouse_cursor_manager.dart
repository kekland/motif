...
class ExclusiveMouseCursorManager extends MouseCursorManager {
  ExclusiveMouseCursorManager(super.fallbackMouseCursor);

  @override
  MouseCursor? debugDeviceActiveCursor(int device) {
    if (_exclusiveSessions[device] != null) {
      return _exclusiveSessions[device]!.cursor;
    }

    return super.debugDeviceActiveCursor(device);
  }

  MouseCursor? deviceExclusiveCursor(int? device) {
    final _device = device ?? _devices.firstOrNull;
    return _exclusiveSessions[_device]?.cursor;
  }

  final _lastCursors = <int, MouseCursor>{};
  final _lastSession = <int, MouseCursorSession>{};
  final _exclusiveSessions = <int, MouseCursorSession>{};
  final _devices = <int>{};

  static MouseCursor? _firstNonDeferred(Iterable<MouseCursor> cursors) {
  ...
  void handleDeviceCursorUpdate(
    ...
    if (triggeringEvent is PointerAddedEvent) {
      _devices.add(device);
  ...
  void _activate(int device) {
    final cursor = _lastCursors[device];
    if (cursor == null) return;
    _lastSession[device]?.dispose();

    final nextSession = cursor.createSession(device);
...
// ---------------------------------------------------------------------------------------------------------------------
// Following code is from the Flutter SDK, with only modification being:
// - `MouseTracker` -> `CustomMouseTracker`
// - `MouseTracker` gains a constructor field `mouseCursorManager`
// ---------------------------------------------------------------------------------------------------------------------
...
class CustomMouseTracker with ChangeNotifier implements MouseTracker {
  ...
  final MouseCursorManager _mouseCursorMixin;
  MouseCursorManager get mouseCursorManager => _mouseCursorMixin;
