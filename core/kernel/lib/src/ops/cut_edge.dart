    ...
    return _compositeOp(
      ...
      () {
        ...
        for (var i = 0; i < ts.length; i++) {
          final id = targetId.derive('$tag.v$i');
        ...
        for (var i = 0; i < pieces.length; i++) {
          ...
          final eId = targetId.derive('$tag.e$i');
        ...
        // Remove original edge and record lineage
        deleteEdge(target);

        final produced = <CellKey>[
          for (final e in eHandles) bundle.edgeKey(e),
          for (final v in vHandles) bundle.vertexKey(v),
        ];

        delta.addLineage(.edge(targetId), produced);
        return MultiCutEdgeResult(vHandles, eHandles, original);
      },
    );
  }
}
