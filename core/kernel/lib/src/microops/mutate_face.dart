...
extension MutateFaceTransaction on TopologyTransaction {
  FaceHandle addFace(
    ...
    ops.add(AddFaceOp(id, boundary.map((c) => c.asRef(bundle)).toList(), placement: .of(bundle, faceHandle)));
    delta.markAdded(.face(id));
    return faceHandle;
  }
  ...
  void deleteFace(FaceHandle f) {
    ...
    ops.add(DeleteFaceOp(id, bundle.faceBoundary(f).map((c) => c.asRef(bundle)).toList(), placement: .of(bundle, f)));
    delta.markDeleted(.face(id));
