...
extension type const CellView._((TopologyBundle b, CellHandle h) k) {
  ...
  FrameView? get parent => bundle.parentOf(handle)?.asView(bundle);
  CellView? get prevSibling => bundle.siblingPrevOf(handle)?.asView(bundle);
  CellView? get nextSibling => bundle.siblingNextOf(handle)?.asView(bundle);

  FrameView lca(CellView other) => bundle.lca(handle, other.handle).asView(bundle);
...
extension type const FrameView._((TopologyBundle b, FrameHandle h) k) implements CellView {
  ...
  Iterable<CellView> get children => bundle.frameChildren(handle).map((h) => h.asView(bundle));
}
...
extension type const VertexView._((TopologyBundle b, VertexHandle h) k) implements CellView {
  ...
  Iterable<EdgeView> get edges => bundle.vertexEdges(handle).map((h) => h.asView(bundle));
  List<List<EdgeView>> get sectors {
    final sectors = <List<EdgeView>>[];
    for (final sector in bundle.vertexSectors(handle)) {
      sectors.add(sector.map((h) => h.asView(bundle)).toList());
    }
    return sectors;
  }
}
...
extension type const EdgeView._((TopologyBundle b, EdgeHandle h) k) implements CellView {
  ...
  VertexView get start => bundle.edgeStart(handle).asView(bundle);
  VertexView get end => bundle.edgeEnd(handle).asView(bundle);
  ...
  Iterable<(FaceView, Coedge)> get uses => bundle.edgeUses(handle).map((u) => (u.$1.asView(bundle), u.$2));
  Iterable<FaceView> get faces => bundle.edgeFaces(handle).map((h) => h.asView(bundle));
}
