...
final class DrawOrderIndex {
  ...
  List<(CellRef, CellHandle)> ofRef(FrameRef frame) => of(bundle.handle(frame)!);
  ...
  void invalidateFrame(FrameRef? frame) {
    _drawOrder.remove(frame ?? CellRef.root);
    _drawIndex = null;
  }
  ...
  List<(CellRef, CellHandle)> _resolve(FrameHandle frame) {
    ...
    for (final c in bundle.frameChildren(frame)) {
      if (bundle.isCellLive(c)) sorted.add(c.ref(bundle));
    }

    sorted.sort((a, b) {
      final s = evaluation.evalOrder(a.statementId, b.statementId);
      if (s != 0) return s;
...
extension EvaluationZOrder on EvalPass {
  void reorder(CellRef r) {
    evaluation.drawOrder.invalidateFrame(frameOf(r));
    reordered.add(r);
  }
}
