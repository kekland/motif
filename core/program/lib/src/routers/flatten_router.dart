...
extension RouteFlatten on Evaluation {
  ProgramEdit routeFlatten(Iterable<StatementId> ids) {
    ...
    while (work.isNotEmpty) {
      ...
      for (final consumed in consumedOf(id)) {
        ...
        descendants[consumed] = lineage.descendantsOf(consumed).where(bundle.isLive).toList();
      }
    }

    final bake = <CellRef>{};
    for (final s in dead) {
      bake.addAll(productsOf(s).where(live.contains));
    }

    final baked = routeBake(bake);
    final remap = Remap.empty()..add(baked.remap);
    for (final entry in descendants.entries) {
      remap[entry.key] = [for (final p in entry.value) baked.remap.cell(p)?.single ?? p];
    }
