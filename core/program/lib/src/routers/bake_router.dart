...
extension RouteBake on Evaluation {
  BakeRouter routeBake(
    ...
    FrameHandle spaceOf(CellHandle h) {
      for (final f in bundle.ancestorsOf(h)) {
        if (!_gone.contains(f.ref(bundle))) return f;
      }
      return bundle.root;
    }
    ...
    for (final f in bakeByKind[CellKind.frame]!) {
      ...
      if (statement(f.statementId) is! GroupStatement) {
        _gone.add(f);
        continue;
      }

      final s = GroupStatement(parent: parentOf(h));
      out.add(s);
      refMap[f] = [s.frame];
    }
    ...
    return .new(statements: out, remap: .cells(refMap));
  }
}
