...
extension _CellRefDecode on gen.CellRef {
  CellRef<H> decode<H extends CellHandle>() {
    ...
    return .make(namespace: namespace.decode(), tag: tag, sub: sub, kind: kind.decode());
  }
}

final _cellRefCodec = _codec<CellRef, gen.CellRef>(
  decoder: (v) => .make(namespace: v.namespace.decode(), tag: v.tag, sub: v.sub, kind: v.kind.decode()),
  encoder: (v) => .new(namespace: v.namespace.encode(), tag: v.tag, sub: v.sub, kind: v.kind.encode()),
);
